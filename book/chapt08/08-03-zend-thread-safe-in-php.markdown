...
全局变量，在函数外声明，例如，`int gVar;`。
全局变量，所有函数共享，在任何地方出现这个变量名都是指这个变量。
...
于是还缺少一种同一个线程中的所有函数共享，不同线程中的函数不共享的变量。
就是说，线程1调用函数`fnA,fnB`中的同名变量是同一个变量（如上假设任何地方不会声明重名的变量）。
而线程2调用函数`fnA,fnB`的时候，虽然他们是同一个变量,但是与线程1中的那个变量不是同一个变量。
C语言里是不提供这样的变量作用域，因此需要代码中自己实现。

PHP可以运行在单线程和多线程的模式下，因此PHP必然需要实现一个上述的作用域。
PHP作为一个开源项目，必然具有可扩展性。显然写PHP扩展的人不应该也不希望去修改PHP的核心代码。
也希望尽量少的涉及PHP核心代码的实现以及PHP会运行在单线程还是多线程模式下。
而TSRM 就是解决上述作用域的问题和程序员的需求的。
